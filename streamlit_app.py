...
try:
    # قراءة البيانات مع تحديث آلي للتخلص من التخزين المؤقت
    df = pd.read_csv(CSV_URL)
    
    if df.empty:
        st.warning("الجدول فارغ حالياً، بانتظار بيانات جديدة.")
    else:
        # عرض كل صف ككرت مستقل
        for _, row in df.iterrows():
            with st.container():
                ...
                <div class="property-card">
                    ...
                    <p class="date-text">📅 توقيت الطلب: {row['Submission_Date']}</p>
    ...
    st.info("بانتظار وصول أول بيانات للمنظومة... تأكد من إرسال تجربة عبر الرابط الخاص بك.")
