...
# --- جلب وعرض البيانات مع الترتيب الذكي ---
try:
    ...
    if not df.empty:
        ...
        c_time = find_c(['time', 'timestamp', 'التاريخ'])
        ...
        # الترتيب
        df['t_b'] = df[c_budget].apply(get_num)
        df['t_t'] = pd.to_datetime(df[c_time], errors='coerce')
        df_display = df.sort_values(by=['t_b', 't_t'], ascending=[False, False])
        
        for _, row in df_display.iterrows():
            def v(k):
                for c in row.index:
                    if k.lower() in str(c).lower(): return str(row[c])
                return "N/A"
            ...
            p_num = str(v('Phone')).replace('.0','')
            st.link_button("واتساب 💬", f"https://wa.me/{p_num}", use_container_width=True)
